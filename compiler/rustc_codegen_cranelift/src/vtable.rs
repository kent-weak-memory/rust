...
pub(crate) fn drop_fn_of_obj(fx: &mut FunctionCx<'_, '_, '_>, vtable: Value) -> Value {
    let slot_size = fx.layout_of(fx.tcx.types.pointer).memory_size.bytes() as usize;
    fx.bcx.ins().load(
        fx.pointer_type,
        vtable_memflags(),
        vtable,
        (ty::COMMON_VTABLE_ENTRIES_DROPINPLACE * slot_size) as i32,
    )
}
...
pub(crate) fn size_of_obj(fx: &mut FunctionCx<'_, '_, '_>, vtable: Value) -> Value {
    ...
    fx.bcx.ins().load(
        ...
        vtable_memflags(),
        vtable,
        (ty::COMMON_VTABLE_ENTRIES_SIZE * slot_size) as i32,
    )
}
...
pub(crate) fn min_align_of_obj(fx: &mut FunctionCx<'_, '_, '_>, vtable: Value) -> Value {
    ...
    fx.bcx.ins().load(
        ...
        vtable_memflags(),
        vtable,
        (ty::COMMON_VTABLE_ENTRIES_ALIGN * slot_size) as i32,
    )
}
...
pub(crate) fn get_ptr_and_method_ref<'tcx>(
    ...
    let slot_size = fx.layout_of(fx.tcx.types.pointer).memory_size.bytes();
    let func_ref = fx.bcx.ins().load(
        fx.pointer_type,
        vtable_memflags(),
        vtable,
        (idx * slot_size as usize) as i32,
    );
    (ptr, func_ref)
}
