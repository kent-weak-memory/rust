...
use rustc_target::abi::{AddressSpace, Align, Size};
...
#[inline]
fn memory_size_and_align_of(ty_and_layout: TyAndLayout<'_>) -> (Size, Align) {
    (ty_and_layout.memory_size, ty_and_layout.align.abi)
}
...
fn build_fixed_size_array_di_node<'ll, 'tcx>(
    ...
    let (memory_size, align) = cx.memory_size_and_align_of(array_type);
...
fn build_pointer_or_reference_di_node<'ll, 'tcx>(
    ...
    debug_assert_eq!(
        cx.memory_size_and_align_of(ptr_type),
        cx.memory_size_and_align_of(Ty::new_mut_ptr(cx.tcx, pointee_type))
    );
    ...
    // There may be a better way to detect CHERI.
    let ptr_address_space = match cx.tcx.data_layout.data_address_space {
    ...
    match fat_pointer_kind(cx, pointee_type) {
        None => {
            ...
            debug_assert_eq!(
                (data_layout.pointer_memory_size, data_layout.pointer_align.abi),
                cx.memory_size_and_align_of(ptr_type),
            ...
            let di_node = unsafe {
                llvm::LLVMRustDIBuilderCreatePointerType(
                    ...
                    data_layout.pointer_memory_size.bits(),
                ...
                |cx, owner| {
                    ...
                    smallvec![
                        build_field_di_node(
                            ...
                            owner,
                            addr_field_name,
                            (addr_field.memory_size, addr_field.align.abi),
                        ...
                        build_field_di_node(
                            ...
                            owner,
                            extra_field_name,
                            (extra_field.memory_size, extra_field.align.abi),
...
fn build_subroutine_type_di_node<'ll, 'tcx>(
    ...
    let di_node = unsafe {
        llvm::LLVMRustDIBuilderCreatePointerType(
            DIB(cx),
            fn_di_node,
            cx.tcx.data_layout.pointer_memory_size.bits(),
    ...
    if let ty::Dynamic(..) = dyn_type.kind() {
        ...
        type_map::build_type_with_children(
            ...
            type_map::stub(
                ...
                unique_type_id,
                &type_name,
                cx.memory_size_and_align_of(dyn_type),
                NO_SCOPE_METADATA,
                DIFlags::FlagZero,
            ),
            |_, _| smallvec![],
...
fn build_field_di_node<'ll, 'tcx>(
    ...
    owner: &'ll DIScope,
    name: &str,
    memory_size_and_align: (Size, Align),
    offset: Size,
    flags: DIFlags,
    ...
    unsafe {
        llvm::LLVMRustDIBuilderCreateMemberType(
            ...
            unknown_file_metadata(cx),
            UNKNOWN_LINE_NUMBER,
            memory_size_and_align.0.bits(),
            memory_size_and_align.1.bits() as u32,
            offset.bits(),
            flags,
            type_di_node,
        )
...
fn build_struct_type_di_node<'ll, 'tcx>(
    ...
    type_map::build_type_with_children(
        ...
        type_map::stub(
            ...
            memory_size_and_align_of(struct_type_and_layout),
    ...
    up_var_tys
        ...
        .map(|(index, (up_var_ty, capture_name))| {
            build_field_di_node(
                ...
                capture_name.as_str(),
                cx.memory_size_and_align_of(up_var_ty),
...
fn build_tuple_type_di_node<'ll, 'tcx>(
    ...
    type_map::build_type_with_children(
        ...
        type_map::stub(
            ...
            unique_type_id,
            &type_name,
            memory_size_and_align_of(tuple_type_and_layout),
            NO_SCOPE_METADATA,
            DIFlags::FlagZero,
        ),
        // Fields:
            ...
            component_types
                ...
                .map(|(index, component_type)| {
                    build_field_di_node(
                        ...
                        cx.memory_size_and_align_of(component_type),
...
fn build_closure_env_di_node<'ll, 'tcx>(
    ...
    type_map::build_type_with_children(
        ...
        type_map::stub(
            ...
            unique_type_id,
            &type_name,
            cx.memory_size_and_align_of(closure_env_type),
            Some(containing_scope),
            DIFlags::FlagZero,
        ),
        // Fields:
...
fn build_union_type_di_node<'ll, 'tcx>(
    ...
    type_map::build_type_with_children(
        ...
        type_map::stub(
            ...
            unique_type_id,
            &type_name,
            memory_size_and_align_of(union_ty_and_layout),
            Some(containing_scope),
            DIFlags::FlagZero,
        ),
        // Fields:
            ...
            variant_def
                ...
                .map(|(i, f)| {
                    ...
                    build_field_di_node(
                        ...
                        owner,
                        f.name.as_str(),
                        memory_size_and_align_of(field_layout),
                        Size::ZERO,
                        DIFlags::FlagZero,
                        type_di_node(cx, field_layout.ty),
                    )
...
fn build_vtable_type_di_node<'ll, 'tcx>(
    ...
    let (pointer_memory_size, pointer_align) = cx.memory_size_and_align_of(void_pointer_ty);
    ...
    let size = pointer_memory_size * vtable_entries.len() as u64;
        ...
        |cx, vtable_type_di_node| {
            vtable_entries
                ...
                .filter_map(|(index, vtable_entry)| {
                    ...
                    let field_offset = pointer_memory_size * index as u64;

                    Some(build_field_di_node(
                        cx,
                        vtable_type_di_node,
                        &field_name,
                        (pointer_memory_size, pointer_align),
                        field_offset,
                        DIFlags::FlagZero,
                        field_type_di_node,
                    ))
