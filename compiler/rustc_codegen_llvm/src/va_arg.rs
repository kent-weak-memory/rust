...
fn emit_ptr_va_arg<'ll, 'tcx>(
    ...
    let (llty, size, align) = if indirect {
        (
            ...
            bx.cx.data_layout().pointer_memory_size,
            bx.cx.data_layout().pointer_align,
        )
    } else {
        (layout.llvm_type(bx.cx), layout.memory_size, layout.align)
    };
    let (addr, addr_align) =
...
fn emit_aapcs_va_arg<'ll, 'tcx>(
    ...
    let (reg_off, reg_top_index, slot_size) = if gr_type {
        ...
        let nreg = (layout.memory_size.bytes() + 7) / 8;
        ...
        let nreg = (layout.memory_size.bytes() + 15) / 16;
    ...
    if bx.tcx().sess.target.endian == Endian::Big && layout.memory_size.bytes() != slot_size {
        // On big-endian systems the value is right-aligned in its slot.
        let offset = bx.const_i32((slot_size - layout.memory_size.bytes()) as i32);
