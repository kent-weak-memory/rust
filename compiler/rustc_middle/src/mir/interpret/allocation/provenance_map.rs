...
impl<Prov: Provenance> ProvenanceMap<Prov> {
    fn adjusted_range_ptrs(range: AllocRange, cx: &impl HasDataLayout) -> Range<Size> {
        ...
        let adjusted_start = Size::from_bytes(
            range.start.bytes().saturating_sub(cx.data_layout().pointer_memrepr_size.bytes() - 1),
        );
        adjusted_start..range.end_memrepr()
    }
    ...
    fn range_bytes_get(&self, range: AllocRange) -> &[(Size, Prov)] {
        if let Some(bytes) = self.bytes.as_ref() {
            bytes.range(range.start..range.end_memrepr())
        } else {
            &[]
        }
    }
    ...
    fn range_bytes_is_empty(&self, range: AllocRange) -> bool {
        self.bytes
            .as_ref()
            .is_none_or(|bytes| bytes.range_is_empty(range.start..range.end_memrepr()))
    }

    /// Get the provenance of a single byte.
    pub fn get(&self, offset: Size, cx: &impl HasDataLayout) -> Option<Prov> {
        let prov = self.range_ptrs_get(
            alloc_range(offset, Some(Size::from_bytes(1)), Size::from_bytes(1)),
            cx,
        );
    ...
    pub fn insert_ptr(&mut self, offset: Size, prov: Prov, cx: &impl HasDataLayout) {
        debug_assert!(self.range_empty(
            alloc_range(
                offset,
                Some(cx.data_layout().pointer_data_size),
                cx.data_layout().pointer_memrepr_size
            ),
            cx
        ));
    ...
    pub fn clear(&mut self, range: AllocRange, cx: &impl HasDataLayout) -> AllocResult {
        ...
        let (first, last) = {
            ...
            (
                ...
                provenance.last().unwrap().0 + cx.data_layout().pointer_memrepr_size,
            )
        };
        ...
        if last > end {
            let begin_of_last = last - cx.data_layout().pointer_memrepr_size;
            if !Prov::OFFSET_IS_ADDR {
                // We can't split up the provenance into less than a pointer.
...
impl<Prov: Provenance> ProvenanceMap<Prov> {
    pub fn prepare_copy(
        ...
        let shift_offset = move |idx, offset| {
            ...
            let dest_offset = dest + src.memrepr_size * idx; // `Size` operations
            // shift offsets from source allocation to destination allocation
            (offset - src.start) + dest_offset // `Size` operations
        };
        let ptr_memrepr_size = cx.data_layout().pointer_memrepr_size;
        ...
        if src.memrepr_size >= ptr_memrepr_size {
            let adjusted_end =
                Size::from_bytes(src.end_memrepr().bytes() - (ptr_memrepr_size.bytes() - 1));
        ...
        let begin_overlap =
            self.range_ptrs_get(alloc_range(src.start, None, Size::ZERO), cx).first();
        let end_overlap =
            self.range_ptrs_get(alloc_range(src.end_memrepr(), None, Size::ZERO), cx).first();
        if !Prov::OFFSET_IS_ADDR {
            ...
            if let Some(entry) = begin_overlap {
                ...
                let entry_end = cmp::min(entry.0 + ptr_memrepr_size, src.end_memrepr());
            ...
            if let Some(entry) = end_overlap {
                ...
                for offset in entry_start..src.end_memrepr() {
