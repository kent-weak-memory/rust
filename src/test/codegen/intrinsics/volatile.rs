...
pub unsafe fn volatile_copy_memory(a: *mut u8, b: *const u8) {
    // NONCHERI: llvm.memmove.p0i8.p0i8.{{\w*(.*true)}}
    // CHERI: llvm.memmove.p200i8.p200i8.{{\w*(.*true)}}
...
pub unsafe fn volatile_copy_nonoverlapping_memory(a: *mut u8, b: *const u8) {
    // NONCHERI: llvm.memcpy.p0i8.p0i8.{{\w*(.*true)}}
    // CHERI: llvm.memcpy.p200i8.p200i8.{{\w*(.*true)}}
...
pub unsafe fn volatile_set_memory(a: *mut u8, b: u8) {
    // NONCHERI: llvm.memset.p0i8.{{\w*(.*true)}}
    // CHERI: llvm.memset.p200i8.{{\w*(.*true)}}
