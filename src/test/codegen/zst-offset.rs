...
// CHECK: [[X0:%[0-9]+]] = bitcast i64{{( addrspace\(.*\))?}}* %s to i8{{( addrspace\(.*\))?}}*
// CHECK-NEXT: [[X1:%[0-9]+]] = getelementptr i8, i8{{( addrspace\(.*\))?}}* [[X0]], [[USIZE]] 8
    let x = &s.1;
...
// CHECK: [[X0:%[0-9]+]] = bitcast { i64, i32 }{{( addrspace\(.*\))?}}* %s to i8{{( addrspace\(.*\))?}}*
// CHECK-NEXT: [[X1:%[0-9]+]] = getelementptr i8, i8{{( addrspace\(.*\))?}}* [[X0]], [[USIZE]] 12
    let x = &s.2;
    &x; // keep variable in an alloca
}
...
// CHECK: [[X0:%[0-9]+]] = bitcast <4 x i64>{{( addrspace\(.*\))?}}* %s to i8{{( addrspace\(.*\))?}}*
// CHECK-NEXT: [[X1:%[0-9]+]] = getelementptr i8, i8{{( addrspace\(.*\))?}}* [[X0]], [[USIZE]] 32
    let x = &s.1;
    &x; // keep variable in an alloca
}
