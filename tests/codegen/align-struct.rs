...
// NONCHERI: call void @llvm.memcpy.{{.*}}({{i8\*|ptr}} align 64 %{{.*}}, {{i8\*|ptr}} align 64 %{{.*}}, i{{[0-9]+}} 64, i1 false)
// CHERI: call void @llvm.memcpy.{{.*}}({{i8 addrspace\(200\)\*|ptr addrspace\(200\)}} align 64 %{{.*}}, {{i8 addrspace\(200\)\*|ptr addrspace\(200\)}} align 64 %{{.*}}, i{{[0-9]+}} 64, i1 false)
    let a64 = Align64(i);
    a64
}
...
// NONCHERI: {{%.*}} = load i32, {{i32\*|ptr}} {{%.*}}, align 64
// CHERI: {{%.*}} = load i32, {{i32 addrspace\(200\)\*| addrspace\(200\)ptr}} {{%.*}}, align 64
    a.0
}
