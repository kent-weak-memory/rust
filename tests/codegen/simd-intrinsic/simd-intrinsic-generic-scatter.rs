...
pub unsafe fn scatter_pf32x2(pointers: Vec2<*mut *const f32>, mask: Vec2<i32>,
    ...
    // NONCHERI: call void @llvm.masked.scatter.v2p0{{.*}}.v2p0{{.*}}(<2 x {{float\*|ptr}}> {{.*}}, <2 x {{float\*\*|ptr}}> {{.*}}, i32 {{.*}}, <2 x i1> {{.*}})
    // CHERI: call void @llvm.masked.scatter.v2p200{{.*}}.v2p200{{.*}}(<2 x {{float addrspace\(200\)\*|ptr addrspace\(200\)}}> {{.*}}, <2 x {{float addrspace\(200\)\* addrspace\(200\)\*|ptr addrspace\(200\)}}> {{.*}}, i32 {{.*}}, <2 x i1> {{.*}})
    simd_scatter(values, pointers, mask)
}
